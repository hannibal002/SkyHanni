...
object EntityEvents {
    ...
    fun onEntityRenderPre(event: RenderLivingEvent.Pre<*>) {
        if (HanniRenderEntityEvent.Pre(event.entity, event.x, event.y, event.z).post()) {
            event.isCanceled = true
        }
    }
    ...
    fun onEntityRenderPost(event: RenderLivingEvent.Post<*>) {
        HanniRenderEntityEvent.Post(event.entity, event.x, event.y, event.z).post()
    }

    @SubscribeEvent
    fun onEntityRenderSpecialsPre(event: RenderLivingEvent.Specials.Pre<*>) {
        if (HanniRenderEntityEvent.Specials.Pre(event.entity, event.x, event.y, event.z).post()) {
            event.isCanceled = true
        }
    }
    ...
    fun onEntityRenderSpecialsPost(event: RenderLivingEvent.Specials.Post<*>) {
        HanniRenderEntityEvent.Specials.Post(event.entity, event.x, event.y, event.z).post()
    }
}
