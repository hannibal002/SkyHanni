...
object ForagingTracker : SkyHanniBucketedItemTracker<ForagingTracker.TreeType, ForagingTracker.BucketData>(
    ...
    private fun isHoldingAxe() = InventoryUtils.getItemInHand()?.getItemCategoryOrNull() == AXE || hasHeldAxe
    ...
    private fun onItemAdd(event: ItemAddEvent) {
        if (!isInIsland() || event.source != COMMAND) return
    ...
    private fun onRepoReload(event: RepositoryReloadEvent) {
        ...
        dropsJsonCategories = buildList {
            add(DropCategoryData(UNCOMMON_DROPS, dropsJson.uncommonDrops))
            add(DropCategoryData(ENCHANTED_BOOKS, dropsJson.enchantedBooks))
            add(DropCategoryData(BOOSTERS, dropsJson.boosters))
            add(DropCategoryData(SHARDS, dropsJson.shards))
            add(DropCategoryData(RUNES, dropsJson.runes))
            add(DropCategoryData(MISC, dropsJson.miscDrops))
        }
    }
    ...
    private fun SkyHanniChatEvent.Allow.tryReadLoot() {
        ...
        mobSpawnPattern.matchMatcher(cleanMessage) {
            ...
            if (dropsJson.mobs.contains(mob) && config.compactGiftBonusDropsList.contains(MOBS)) {
    ...
    private fun onItemInHandChange(event: ItemInHandChangeEvent) {
        if (!isInIsland()) return
        val isAxe = event.newItem.getItemStack().getItemCategoryOrNull() == AXE
        if (isAxe != hasHeldAxe) {
            if (!isAxe) {
