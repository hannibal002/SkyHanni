...
class MenuItemDisplayOverlayCombat {
    private val bestiarySpecificPercentPattern = ".* (§.)?(?<percent>[0-9]+)(\.[0-9]*)?(§.)?%".toPattern()
            ...
            if ((chestName.contains("Bestiary")) && !(itemName.isEmpty())) {
                ...
                for (line in lore) {
                    if (line.contains("Families Completed: ") || line.contains("Overall Progress: ")) {
                        return bestiarySpecificPercentPattern.matchMatcher(line.replace(" (MAX!)","")) { group("percent").replace("100", "§a✔") } ?: ""
                    }
                }
            }
        }
        ...
        if (stackSizeConfig.contains(4) && itemName.contains("RNG Meter")) {
            for (line in item.getLore()) {
                if (line.contains("Progress: ")) {
                    return bestiarySpecificPercentPattern.matchMatcher(line) { group("percent").replace("100", "§a✔") } ?: ""
                }
            }
        }
