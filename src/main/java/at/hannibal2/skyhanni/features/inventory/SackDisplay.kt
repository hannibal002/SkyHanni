...
object SackDisplay {
    ...
    private fun drawDisplay(savingSacks: Boolean): List<List<Any>> {
        val (newDisplay, totalPrice) = drawNormalList(savingSacks)
        drawOptions(newDisplay, totalPrice)
        drawRunesDisplay(newDisplay)
        drawGemstoneDisplay(newDisplay, totalPrice)
        return newDisplay
    }

    private fun drawNormalList(savingSacks: Boolean): Pair<MutableList<List<Any>>, Long> {
        SackAPI.getSacksData(savingSacks)
        val sackItems = SackAPI.sackItem.toList()
        if (sackItems.isEmpty()) return Pair(mutableListOf(), 0L)
        ...
        list.addAsSingletonList("§7Items in Sacks: §o(Rendering $amountShowing of ${sortedPairs.size} items)")
        ...
        if (SackAPI.isTrophySack) list.addAsSingletonList("§cTotal Magmafish: §6${totalMagmaFish.addSeparators()}")
        return Pair(list, totalPrice)
    }
    ...
    private fun drawGemstoneDisplay(newDisplay: MutableList<List<Any>>, oldTotalPrice: Long) {
        if (SackAPI.gemstoneItem.isEmpty()) return
        newDisplay.addAsSingletonList("§7Gemstones:")
        var totalPrice = oldTotalPrice
