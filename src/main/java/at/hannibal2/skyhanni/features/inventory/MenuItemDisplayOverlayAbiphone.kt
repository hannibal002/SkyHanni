...
class MenuItemDisplayOverlayAbiphone {
    private val outOfNoColorPattern = ".*: (§.)?(?<useful>[0-9]+)(§.)?\/(?<total>[0-9]+)".toPattern()
        ...
        if ((stackSizeConfig.contains(0)) && (chestName.contains("Abiphone")) && (itemName == ("Contacts Directory"))) {
            for (line in item.getLore()) {
                if (line.contains("Your contacts: ")) {
                    return outOfNoColorPattern.matchMatcher(line) { group("useful") } ?: ""
                }
            }
        }
        ...
        if ((stackSizeConfig.contains(1)) && ((chestName.contains("Abiphone"))) && (itemName.contains(" Operator Chip"))) {
            ...
            for (line in item.getLore()) {
                if (line.contains("Progress to Complete Category")) {
                    ...
                    else return outOfNoColorPattern.matchMatcher(line) { group("useful") } ?: ""
                }
            }
        }
