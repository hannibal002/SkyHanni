...
object WhereWillIBe {
    ...
    private val privateOrGuest = listOf(
        IslandType.PRIVATE_ISLAND_GUEST, IslandType.PRIVATE_ISLAND, IslandType.GARDEN_GUEST
    )
    private val onceOnlyIslands = listOf(
        IslandType.CATACOMBS, IslandType.DARK_AUCTION, IslandType.KUUDRA_ARENA, IslandType.THE_RIFT
    )
    ...
    fun onMessageSendToServer(event: MessageSendToServerEvent) {
        ...
        repeat(Random.nextInt(2, 11)) {
            ...
            islandsAsList.removeIfNecessary(lastIsland)
            ChatUtils.chat(
    ...
    private fun randomMillis(lastUsedMillis: Long): Long = Random.nextLong(
        lastUsedMillis, lastUsedMillis + Random.nextLong(10000, 2.0.pow(26).toLong())
    )

    private fun chooseIsland(lastIsland: IslandType, chosenIslandsList: List<IslandType>): IslandType {
        var islandToReturn = IslandType.NONE
        while (islandToReturn == lastIsland || islandToReturn in nonIslands) {
            islandToReturn = chosenIslandsList.random()
        }
        return islandToReturn
    }

    private fun MutableList<IslandType>.removeIfNecessary(lastIsland: IslandType) {
        if (lastIsland in onceOnlyIslands) this.remove(lastIsland)
    }

    private fun randomIsland(chosenIsland: IslandType): String = "SkyBlock (${chosenIsland.displayName})"
    ...
    private fun miniOrMega(chosenIsland: IslandType): String =
        if (chosenIsland == IslandType.HUB && Random.nextBoolean()) "mega" else "mini"

    private fun randServerNumber(): Int = Random.nextInt(10, 401)

    private fun oneOrTwoLetters(charRange: CharRange = ('A'..'Z')): String =
        if (Random.nextBoolean()) "${charRange.random()}" else "${charRange.random()}${charRange.random()}"

    private fun formattedDate(date: Date): String = SimpleDateFormat("HH:mm:ss dd-MM-yy").format(date)
}
