...
class ChestValue {
    ...
    fun onInventoryOpen(event: InventoryOpenEvent) {
        ...
        var value = 0.0
        val valueMap = mutableMapOf<ItemStack, Double>()

        for (item in event.inventoryItems.values) {
            value += EstimatedItemValue.getEstimatedItemPrice(item, mutableListOf()).first * item.stackSize
            valueMap.addOrPut(
                item,
                EstimatedItemValue.getEstimatedItemPrice(item, mutableListOf()).first * item.stackSize
            )
        }
    ...
    fun onInventoryRender(event: DrawScreenAfterEvent) {
        ...
        val string = if (config.formatItemValueChest) EnumChatFormatting.BOLD.toString() + NumberUtil.format(
            itemValueTotal.round(1)
        ...
        while (i <= itemEntryList.size - 1) {
            ...
            if (entry.key.stackSize != 1) {
                fontRenderer.drawString(
                    entry.key.stackSize.toString(),
                    guiLeft + xSize + 20,
                    guiTop + 30 + (fontRenderer.FONT_HEIGHT * (i + 1)) * 2,
                    Color.WHITE.rgb
                )
            }
            if (config.formatItemValueChest) fontRenderer.drawString(
                EnumChatFormatting.BOLD.toString() + NumberUtil.format(
                    entry.value.round(1)
                ),
                guiLeft + xSize + 10 + (fontRenderer.FONT_HEIGHT * 2),
                guiTop + 20 + (fontRenderer.FONT_HEIGHT * (i + 1)) * 2,
                16766208
            )
            else fontRenderer.drawString(
                EnumChatFormatting.BOLD.toString() + entry.value.round(1),
                guiLeft + xSize + 10 + (fontRenderer.FONT_HEIGHT * 2),
                guiTop + 20 + (fontRenderer.FONT_HEIGHT * (i + 1)) * 2,
                16766208
            )
