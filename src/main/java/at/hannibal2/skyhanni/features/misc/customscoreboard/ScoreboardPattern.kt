...
object ScoreboardPattern {
    ...
    val cleanUpPattern by farmingSb.pattern("cleanup", "^\\s*§fCleanup§7: (§.)*[\\d,]+%$")
    val pastingPattern by farmingSb.pattern("pasting", "^\\s*§f(Barn )?Pasting§7: (§.)*[\\d,]+%$")
    ...
    val powderPattern by miningSb.pattern("powder", "(§.)*᠅ §f(Gemstone|Mithril)( Powder)?(§.)*:?.*$")
    val windCompassPattern by miningSb.pattern("windcompass", "§9Wind Compass$")
    val windCompassArrowPattern by miningSb.pattern("windcompassarrow", "( )*((§[a-zA-Z0-9]|[⋖⋗≈])+)( )*((§[a-zA-Z0-9]|[⋖⋗≈])+)?( )*")
    ...
    val raffleTicketsPattern by miningSb.pattern("raffletickets", "^Tickets: §a\\d+ §7\\(\\d{1,3}\\.\\d%\\)$")
    val rafflePoolPattern by miningSb.pattern("rafflepool", "^Pool: §6\\d+§8/500$")
    val mithrilUselessPattern by miningSb.pattern("mithriluseless", "^§7Give Tasty Mithril to Don!$")
    val mithrilRemainingPattern by miningSb.pattern("mithrilremaining", "^Remaining: §a(\\d+ Tasty Mithril|FULL)$")
    val mithrilYourMithrilPattern by miningSb.pattern("mithrilyourmithril", "^Your Tasty Mithril: §c\\d+.*$")
    ...
    val spookyPattern by eventsSb.pattern("spooky", "§6Spooky Festival§f \\d{0,2}?:?\\d{2}$")
    val winterEventStartPattern by eventsSb.pattern("wintereventstart", "(§.)*Event Start: §.\\d+:\\d+$")
    val winterNextWavePattern by eventsSb.pattern("wintereventnextwave", "(§.)*Next Wave: (§.)*(\\d+:\\d+|Soon!)$")
