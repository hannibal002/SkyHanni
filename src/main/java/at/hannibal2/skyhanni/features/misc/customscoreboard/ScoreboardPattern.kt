...
object ScoreboardPattern {
    ...
    val dojoChallengePattern by miscSb.pattern("dojochallenge", "^Challenge: §.(Control|Tenacity|Swiftness|Disclipline|Mastery|Stamina|Force)$")
    val dojoDifficultyPattern by miscSb.pattern("dojodifficulty", "^Difficulty: §.(Easy|Medium|Hard|Extreme)$")
    val dojoPointsPattern by miscSb.pattern("dojopoints", "^Points: §a\\w*$")
    val dojoTimePattern by miscSb.pattern("dojotime", "^Time: §a\\d{1,2}(\\.\\d)?.*$")
    // events
