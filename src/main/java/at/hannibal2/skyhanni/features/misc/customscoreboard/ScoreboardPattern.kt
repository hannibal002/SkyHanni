...
object ScoreboardPattern {
    ...
    val clearedPattern by dungeonSb.pattern("cleared", "^Cleared: §c\\d{1,3}% §7\\(§b\\d{1,3}§7\\)$")
    ...
    val mithrilPowderPattern by tablistGroup.pattern("mithrilpowder", "^ §fMithril Powder: §2(?<mithrilpowder>\\d*,?\\.?\\d+([a-zA-Z])?)\$")
    val gemstonePowderPattern by tablistGroup.pattern("gemstonepowder", "^ §fGemstone Powder: §d(?<gemstonepowder>\\d*,?\\.?\\d+([a-zA-Z])?)\$")
}
