...
object UpdateManager {
    ...
    fun checkUpdate(force: Boolean = false, forcedUpdateStream: UpdateStream = config.updateStream.get()) {
        ...
        if (forcedUpdateStream == BETA && config.updateStream.get() != BETA) {
            config.updateStream.set(BETA)
        }
    ...
    private fun updateCommand(arg: String) {
        ...
        val updateStream = when {
            arg.matches(releaseStreamPattern) -> RELEASES
            arg.matches(betaStreamPattern) -> BETA
            else -> currentStream
        }

        val switchingToBeta = updateStream == BETA && (currentStream != BETA || !SkyHanniMod.isBetaVersion)
        if (switchingToBeta) {
            ChatUtils.clickableChat(
