...
class SpawnTimers {
    ...
    fun onChatPacket(event: PacketEvent.ReceiveEvent) {
        ...
        if (packet is S2APacketParticles) {
            ...
            if (arachneAltarLocation.distance(location) > 10) return
            if (packet.particleType == EnumParticleTypes.REDSTONE && packet.particleSpeed == 1.0f) {
                particleCounter += 1
            } else {
                LorenzUtils.warning("Unsuitable particle: ${packet.particleType}/${packet.particleSpeed}")
            }
        }
    }
