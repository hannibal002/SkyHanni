...
class CanePage: FFGuideGUI.FFGuidePage() {
    private val tool = GardenAPI.config?.fortune?.farmingItems?.get(7)?.let { NEUItems.loadNBTData(it) }
    ...
    override fun drawPage(mouseX: Int, mouseY: Int, partialTicks: Float) {
        if (FFGuideGUI.breakdownMode) {
            ...
            FFGuideGUI.textLinesWithTooltip[Pair("§6Sugar Cane Farming Fortune", "")] = Pair(FFGuideGUI.guiLeft + 140,  FFGuideGUI.guiTop + 5)
            ...
            FFGuideGUI.textLinesWithTooltip[Pair("§2Collection Analysts", "§7§2Fortune from increasing crop counter\n" +
            ...
            FFGuideGUI.textLinesWithTooltip[Pair("§2Turbo-Cane Enchantment", "§7§2Fortune for each enchantment level\n" +
