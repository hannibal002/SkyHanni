...
object PestSpawn {
    ...
    private val onePestPattern by patternGroup.list(
        "one.colorless",
        "^\\w+! A ${SkyblockMobType.PEST.hypixelIcon} Pest has appeared in Plot - (?<plot>.*)!",
        "^\\w+! A ${SkyblockMobType.PEST.hypixelIcon} Pest has appeared in (?<plot>The Barn)!",
    )
    ...
    private val multiplePestsPattern by patternGroup.list(
        "multiple.colorless",
        "^\\w+! (?<amount>\\d) ${SkyblockMobType.PEST.hypixelIcon} Pests? have spawned in Plot - (?<plot>.*)!",
        "^\\w+! (?<amount>\\d) ${SkyblockMobType.PEST.hypixelIcon} Pests? have spawned in (?<plot>The Barn)!",
    )
    ...
    private val offlinePestsPattern by patternGroup.pattern(
        "offline.colorless",
        "^\\w+! While you were offline, ${SkyblockMobType.PEST.hypixelIcon} Pests? spawned in Plots (?<plots>.*)!",
    )
