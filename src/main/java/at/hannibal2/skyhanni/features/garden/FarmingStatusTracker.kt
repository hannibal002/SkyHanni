...
object FarmingStatusTracker {
    ...
    private fun formatPlayerSkin(part: SkinBodyPart, scale: Int = 12) =
        "https://api.mineatar.io/${part.path}/${PlayerUtils.getUuid()}?scale=$scale"

    // Prepares and sends the embed to the configured webhook
    suspend fun prepareAndSendEmbed(status: String): Boolean {
        playerFaceURL = playerFaceURL.ifBlank { formatPlayerSkin(config.embed.bodyPart) }
    ...
    private suspend fun sendOrEditMessage(embed: Embed, threadID: String?, username: String): Boolean {
