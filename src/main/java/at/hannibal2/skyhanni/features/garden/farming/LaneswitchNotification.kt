...
class LaneswitchNotification {
    ...
    private fun getFarmBounds(plotIndex: Int, playerPos: LorenzVec, lastPos: LorenzVec): List<LorenzVec> {
        ...
        if (xVelocity.absoluteValue > zVelocity.absoluteValue) {
            val xValueMin = if (plotIndex - 1 == -1 || !plots[plotIndex - 1].owned || plots.indexOf(plots[plotIndex - 1]) == 12 )
                    plots[plotIndex].box.minX else plots[plotIndex - plotIndex % 5].box.minX
            val xValueMax = if ((plotIndex + 1) % 5 == 0 || plotIndex + 1 == 25 || !plots[plotIndex + 1].owned || plots.indexOf(plots[plotIndex + 1]) == 12)
            ...
            val zValueTop = if (plotIndex - 1 == -1 || plotIndex - 5 < 0 || !plots[plotIndex - 5].owned || plots.indexOf(plots[plotIndex - 5]) == 12)
                    plots[plotIndex].box.minZ else plots[plotIndex - 5].box.minZ
            val zValueBottom = if ((plotIndex + 1) % 5 == 0 || plotIndex + 5 > 24 || !plots[plotIndex + 5].owned || plots.indexOf(plots[plotIndex + 5]) == 12)
