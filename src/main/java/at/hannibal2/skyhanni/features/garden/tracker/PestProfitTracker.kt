...
import at.hannibal2.skyhanni.utils.json.JsonUtils.asLongOrNull
...
object PestProfitTracker : SkyHanniBucketedItemTracker<PestType, PestProfitTracker.BucketData>(
    ...
    private fun addKill(type: PestType) {
        ...
        // Moondew is only used in pest traps, so count one per Lunar Moth instead of through spray chat.
        if (type == LUNAR_MOTH) SprayType.MOONDEW.addSprayUsed()
    ...
    private fun onConfigFix(event: ConfigUpdaterMigrator.ConfigFixEvent) {
        ...
        // Retroactively track 1 Moondew spray used per Lunar Moth kill.
        event.transform(147, "#profile.garden.pestProfitTracker") { tracker ->
            ...
            val spraysUsed = trackerObject["spraysUsed"] as? JsonObject
                ?: JsonObject().also { trackerObject.add("spraysUsed", it) }
            spraysUsed.addProperty(SprayType.MOONDEW.name, kills)
            trackerObject
        }
    }
}
