...
object GardenVisitorShoppingList {
    ...
    private val ignoredStates = setOf(
        VisitorApi.VisitorStatus.ACCEPTED,
        VisitorApi.VisitorStatus.REFUSED,
    )
    private fun prepareDrawingData(): Pair<MutableMap<NeuInternalName, Int>, MutableList<VisitorApi.Visitor>> {
        ...
        for (visitor in VisitorApi.getVisitors()) {
            val ignored = (visitor.status in ignoredStates) ||
                (visitor.visitorName.removeColor() == "Spaceman" && config.ignoreSpaceman)
            if (ignored) continue
    ...
    private fun MutableList<Renderable>.drawVisitor(visitor: VisitorApi.Visitor) {
        ...
        if (config.itemPreview) {
            val mapVisitor = GardenVisitorColorNames.visitorMap[visitorName.removeColor()]
            val items = mapVisitor?.needItems
            if (items == null) {
            ...
            if (items.isEmpty()) {
                ...
                } else if (mapVisitor.unknownRewards == true) {
