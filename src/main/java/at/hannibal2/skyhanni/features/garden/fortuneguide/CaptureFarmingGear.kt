...
class CaptureFarmingGear {
    ...
    companion object {
        ...
        fun reverseCarrotFortune() {
            ...
            LorenzUtils.chat("§2Toggled exportable carrot fortune to: ${storage.carrotFortune}", false)
        }

        fun reversePumpkinFortune() {
            val storage = GardenAPI.config?.fortune ?: return
            storage.pumpkinFortune = !storage.pumpkinFortune
            LorenzUtils.chat("§2Toggled expired pumpkin fortune to: ${storage.pumpkinFortune}", false)
        }
    }
