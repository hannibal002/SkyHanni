    ...
    companion object EliteLeaderboardDisplayManager {
        ...
        fun onConfigFix(event: ConfigUpdaterMigrator.ConfigFixEvent) {
            ...
            val displayList: List<FarmingWeightTextEntry> = buildList {
                ...
                event.transform(107, "garden.eliteFarmingWeights.overtakeETA") { entry ->
            ...
            val leaderboardDisplayList: List<EliteLeaderboards> = buildList {
                event.transform(108, "$oldConfig.display") { entry ->
                    if (entry.asBoolean) add(WEIGHT)
                    entry
                }
                event.transform(108, "garden.cropMilestones.progress") { entry ->
                    if (entry.asBoolean) add(CROP)
                    entry
                }
                event.transform(108, "garden.pests.pestProfitTracker.enabled") { entry ->
                    if (entry.asBoolean) add(PEST)
                    entry
                }
            }
            event.add(108, "garden.eliteFarmersLeaderboards.display") {
                ConfigManager.gson.toJsonTree(leaderboardDisplayList)
            }
            event.move(108, "$oldConfig.pos", "garden.eliteFarmersLeaderboards.displayPositions") { entry ->
            ...
            event.move(108, "$oldConfig.showOutsideGarden", "$display.showOutsideGarden")
            event.move(108, "$oldConfig.text", "$display.text")
            event.move(108, "$oldConfig.leaderboard", "$display.leaderboard")
            event.move(108, "$oldConfig.overtakeETA", "$display.overtakeETA")
            event.move(108, "$oldConfig.overtakeETAAlways", "$display.overtakeETA.always")
            event.move(108, "$oldConfig.showLbChange", "$newConfig.showLbChange")
            event.move(108, "$oldConfig.etaGoalRank", "$rankGoal.rankGoal")
            event.move(108, "$oldConfig.etaGoalRank", "$rankGoal.monthlyRankGoal")
            event.move(108, "$oldConfig.etaGoalRank", "$rankGoal.useRankGoal") { entry ->
                ConfigManager.gson.toJsonTree(entry.asString != "10000")
            }
            event.move(108, "$oldConfig.ignoreLow", "$display.ignoreLow")
        }
