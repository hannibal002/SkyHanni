...
class DungeonArchitectFeatures {
    ...
    private val patternGroup = RepoPattern.group("dungeon.architectsdraft.new")
    ...
    fun onChat(event: LorenzChatEvent) {
        ...
        puzzleFailPattern.findMatcher(event.message) {
            generateMessage(group("name"), event)
        }
        quizPuzzleFailPattern.findMatcher(event.message) {
            generateMessage(group("name"), event)
        }
    }
