...
object IslandExceptions {
    ...
    private fun dungeon(
        ...
        baseEntity is IronGolem && MobFilter.wokeSleepingGolemPattern.matches(armorStand?.name.formattedTextCompatLessResets().orEmpty()) ->
            MobData.MobResult.found(Mob(baseEntity, MobType.DUNGEON, armorStand, "Sleeping Golem")) // Consistency fix
    ...
    private fun theRift(
        ...
        baseEntity is Slime && nextEntity is Slime ->
            MobData.MobResult.found(Mob(baseEntity, MobType.SPECIAL, armorStand, "Bacte Tentacle"))

        baseEntity is Slime && armorStand != null && armorStand.cleanName().startsWith("﴾ [Lv10] B") ->
            MobData.MobResult.found(Mob(baseEntity, MobType.BOSS, armorStand, name = "Bacte"))

        baseEntity is RemotePlayer && baseEntity.isNpc() && baseEntity.name.formattedTextCompatLessResets() == "Branchstrutter " ->
            MobData.MobResult.found(Mob(baseEntity, MobType.DISPLAY_NPC, name = "Branchstrutter"))
    ...
    private fun crimsonIsle(
        ...
        baseEntity is RemotePlayer && baseEntity.isNpc() && baseEntity.name.string == "BarbarianGuard " ->
            MobData.MobResult.found(Mob(baseEntity, MobType.DISPLAY_NPC, name = "Barbarian Guard"))

        baseEntity is RemotePlayer && baseEntity.isNpc() && baseEntity.name.string == "MageGuard " ->
            MobData.MobResult.found(Mob(baseEntity, MobType.DISPLAY_NPC, name = "Mage Guard"))

        baseEntity is RemotePlayer && baseEntity.isNpc() && baseEntity.name.string == "Mage Outlaw" ->
            // fix for wierd name
            MobData.MobResult.found(Mob(baseEntity, MobType.BOSS, armorStand, name = "Mage Outlaw"))

        baseEntity is ZombifiedPiglin &&
            baseEntity.getEntityHelmet()?.getSkullTexture() == MobFilter.NPC_TURD_SKULL ->
            MobData.MobResult.found(Mob(baseEntity, MobType.DISPLAY_NPC, name = "Turd"))
    ...
    private fun deepCaverns(baseEntity: LivingEntity) = when {
        baseEntity is Creeper && baseEntity.baseMaxHealth.derpy() == 120 ->
            MobData.MobResult.found(
                Mob(baseEntity, MobType.BASIC, name = "Sneaky Creeper", levelOrTier = 3),
            )
    ...
    private fun crystalHollows(
        ...
        baseEntity is MagmaCube &&
            ...
            MobData.MobResult.found(
                Mob(baseEntity, MobType.BOSS, armorStand, "Bal", levelOrTier = 100),
            )
    ...
    private fun garden(baseEntity: LivingEntity) = when {
        baseEntity is RemotePlayer && baseEntity.isNpc() ->
            MobData.MobResult.found(Mob(baseEntity, MobType.DISPLAY_NPC, name = baseEntity.cleanName()))
    ...
    private fun winterIsland(baseEntity: LivingEntity): MobData.MobResult? {
        ...
        return when {
            baseEntity is MagmaCube &&
                ...
                MobData.MobResult.found(Mob(baseEntity, MobType.BOSS, armorStand, "Jerry Magma Cube"))
    ...
    private fun ratHandler(baseEntity: Zombie, nextEntity: LivingEntity?): MobData.MobResult? =
        generateSequence(RAT_SEARCH_START) { it + 1 }
            ...
            .firstOrNull {
                ...
                MobData.MobResult.found(Mob(baseEntity, mobType = MobType.BASIC, armorStand = it, name = "Rat"))
