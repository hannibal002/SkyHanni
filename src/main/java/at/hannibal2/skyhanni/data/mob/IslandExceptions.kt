...
object IslandExceptions {
    ...
    private fun dungeon(
        ...
        baseEntity is EntityZombie &&
            ...
            MobData.MobResult.found(MobFactories.special(baseEntity, armorStand.cleanName(), armorStand))
        ...
        baseEntity is EntityOtherPlayerMP && baseEntity.isNPC() && baseEntity.name == "The Professor" ->
            MobUtils.getArmorStand(baseEntity, 9).makeMobResult { MobFactories.boss(baseEntity, it) }

        baseEntity is EntityOtherPlayerMP &&
            baseEntity.isNPC() &&
            (nextEntity is EntityGiantZombie || nextEntity == null) &&
            baseEntity.name.contains("Livid") ->
        ...
        baseEntity is EntityIronGolem &&
            MobFilter.wokeSleepingGolemPattern.matches(
                armorStand?.name ?: "",
    ...
    private fun privateIsland(
        ...
        ) MobData.MobResult.found(MobFactories.minionMob(baseEntity)) else MobData.MobResult.notYetFound
    ...
    private fun theRift(
        ...
        baseEntity is EntitySlime &&
            armorStand != null &&
            armorStand.cleanName()
    ...
    private fun crystalHollows(
        ...
        baseEntity is EntityMagmaCube && armorStand != null && armorStand.cleanName() == "[Lv100] Bal ???❤" ->
            MobData.MobResult.found(Mob(baseEntity, Mob.Type.BOSS, armorStand, "Bal", levelOrTier = 100))
    ...
    private fun winterIsland(baseEntity: EntityLivingBase) = when {
        baseEntity is EntityMagmaCube &&
            MobFilter.jerryMagmaCubePattern.matches(
    ...
    private fun ratHandler(baseEntity: EntityZombie, nextEntity: EntityLivingBase?): MobData.MobResult? =
        generateSequence(RAT_SEARCH_START) { it + 1 }.take(RAT_SEARCH_UP_TO - RAT_SEARCH_START + 1).map { i ->
            MobUtils.getArmorStand(baseEntity, i)
        }.firstOrNull {
            it != null &&
