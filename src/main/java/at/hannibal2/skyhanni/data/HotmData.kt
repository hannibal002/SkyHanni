...
enum class HotmData(
    ...
    PEAK_OF_THE_MOUNTAIN("Peak of the Mountain", 7, { null }, { emptyMap() }),
    ...
    companion object {
        ...
        private val heartMithrilPattern by repoGroup.pattern(
            "invetory.heart.mithril", "§7Mithril Powder: §a§2(?<powder>[\\d,]+)"
        )
        private val heartGemstonePattern by repoGroup.pattern(
            "invetory.heart.gemstone", "§7Gemstone Powder: §a§d(?<powder>[\\d,]+)"
        )
        ...
        private val resetMithrilPattern by repoGroup.pattern(
            "invetory.reset.mithril", "\\s+§8- §2(?<powder>[\\d,]+) Mithril Powder"
        )
        private val resetGemstonePattern by repoGroup.pattern(
            "invetory.reset.gemstone", "\\s+§8- §d(?<powder>[\\d,]+) Gemstone Powder"
        )
