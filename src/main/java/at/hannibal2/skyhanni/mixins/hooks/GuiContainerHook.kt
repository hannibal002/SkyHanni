...
class GuiContainerHook(guiAny: Any) {
    ...
    fun shouldCancelMouseClick(slot: Slot?, slotId: Int, clickedButton: Int, clickType: Int): Boolean {
        val item = container.items?.takeIf { it.size > slotId && slotId >= 0 }?.get(slotId)
        return SlotClickEvent(gui, container, item, slot, slotId, clickedButton, ClickType.getTypeById(clickType)).post()
    }
}
