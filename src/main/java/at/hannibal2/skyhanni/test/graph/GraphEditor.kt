...
object GraphEditor {
    ...
    private var lastAutoGeneratedNode: GraphingNode? = null
...
//         autoExpand()
    ...
    private fun autoExpand() {
        val closedNode = closedNode ?: return
        val location = LocationUtils.playerLocation()
        val distance = closedNode.position.add(0.5, 0.5, 0.5).distance(location)

        val inAir = !Minecraft.getMinecraft().thePlayer.onGround
        val maxDistance = if (inAir) 4.5 else 2.5
        if (distance > maxDistance) {
//             if (distance > 10 && closedNode != lastAutoGeneratedNode) {
//                 println("too far away")
//                 return
//             }
            val newNode = GraphingNode(id++, LocationUtils.playerEyeLocation().roundLocationToBlock())
            nodes.add(newNode)
            feedBackInTutorial("Auto expanded a node")
            addEdge(closedNode, newNode)
            lastAutoGeneratedNode = newNode
        }
        if (distance < 3) {
            val lastNode = lastAutoGeneratedNode ?: return
            if (closedNode != lastNode) {
                if (lastNode.position.add(0.5, 0.5, 0.5).distance(location) < 5) {
                    addEdge(lastNode, closedNode)
                }
            }
        }
    }
