...
object WebhookUtils {
    ...
    private suspend fun postPayload(payload: Payload, url: String): JsonObject? =
        ApiUtils.postJSON(url, Gson().toJson(payload), "Discord Webhook Api").data?.asJsonObject

    private suspend fun patchPayload(payload: Payload, url: String): JsonObject? =
        ApiUtils.patchJSON(url, Gson().toJson(payload), "Discord Webhook Api").data?.asJsonObject

    suspend fun sendMessageToWebhook(
        webhookUrl: String,
        message: String = "",
        ...
        val response = postPayload(messagePayload, finalUrl) ?: return false
    ...
    suspend fun sendEmbedsToWebhook(
        webhookUrl: String,
        embeds: List<Embed>,
        threadID: String? = null,
        username: String? = null,
        ...
        val response = postPayload(embedPayload, finalUrl)?.takeIf { it.has("id") }
        lastMessageID = response?.get("id")?.asLong

        ChatUtils.debug("Embeds sent to webhook.")
        return true
    }

    suspend fun editMessageEmbeds(
        webhookUrl: String,
        embeds: List<Embed>,
        threadID: String? = null,
        username: String? = null,
        ...
        val response = if (edit) {
            patchPayload(embedPayload, finalUrl)
        } else {
            postPayload(embedPayload, finalUrl)
        }?.takeIf { it.has("id") }
        lastMessageID = response?.get("id")?.asLong
