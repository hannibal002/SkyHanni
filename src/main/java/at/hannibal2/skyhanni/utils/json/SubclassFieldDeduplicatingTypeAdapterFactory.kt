...
object SubclassFieldDeduplicatingTypeAdapterFactory : TypeAdapterFactory {
    ...
    private val typesInProgress = ThreadLocal.withInitial<MutableSet<String>> { mutableSetOf() }

    override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
        val rawType = type.rawType
        if (!typesInProgress.get().add(rawType.name)) return null
        ...
        return try {
            ...
            typesInProgress.get().remove(rawType.name)
        }
    }
