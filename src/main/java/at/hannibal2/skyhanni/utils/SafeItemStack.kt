...
fun SafeItemStack(item: Item, count: Int = 1): SafeItemStack {
    ...
    //~ if > 1.21.11 'ItemStack(item, count)' -> 'DeferredItemStack(item, { ItemStackTemplate(item, count) }, count)'
...
fun SafeItemStack(item: Item, count: Int = 1, extraOps: SafeItemStack.() -> Unit): SafeItemStack {
    if (count <= 0 || item == Items.AIR) return ItemStack.EMPTY
    //~ if > 1.21.11 'ItemStack(item, count).also(extraOps)' -> 'DeferredItemStack(item, { createItemStackTemplate(item, count, extraOps) }, count)'
