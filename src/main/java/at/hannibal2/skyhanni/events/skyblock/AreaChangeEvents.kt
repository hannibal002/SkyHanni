...
open class AreaChangeEvent(val area: AreaType, val previousArea: AreaType?) : SkyHanniEvent() {
    val isNewArea: Boolean
        get() = area != previousArea
}
 ...
 * data. That data is maintained in the SkyHanni repo, so it covers area borders Hypixel does not map out as clearly
 * on the scoreboard, and it can be corrected without a mod update. Only use this event when graph data is not usable
 * in the given scenario, for example on islands without graph data, or when the scoreboard name itself is the value
 * that is needed.
 *
