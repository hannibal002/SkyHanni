...
class RepoPatternVariableNameStyle(config: Config, private val ctx: RepoPatternContext) :
    SkyHanniRule(config, "All repo pattern variable names must end with 'Pattern' and start with a lowercase letter.") {
    ...
    override fun visitPropertyDelegate(delegate: KtPropertyDelegate) {
        ...
        if (!variableName.first().isLowerCase()) {
            delegate.reportIssue("Repo pattern variable `${variableName}` must start with a lowercase letter.")
        }
    }
}
